  ...
  useEffect(() => {
   ...
   const handleSubmit = async () => {
    try {
      if (editingId) {
         await axios.post(
          `http://localhost:8000/employe/update/${editingId}`,
          formData
        );
      } else {
         await axios.post("http://localhost:8000/employe/add", formData);
      }
      fetchEmployees();
      handleClose();
    } catch (err) {
   ...
   const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8000/employe/delete/${id}`);
      fetchEmployees();
    } catch (err) {
      console.error(err);
    }
